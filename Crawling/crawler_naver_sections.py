...
class crawler_naver_secRank_v2 (crawler_engine):
    def __init__( self , isHidden, outdir, title, keyword , sectionID , startDate, endDate ) :
        self.keyword     = keyword
        super().__init__(isHidden, outdir, title)
        self.sectionID   = sectionID
    ...
    def fetch(self):
        ...
        while _cur < _end:
            self.curDate = _cur.strftime("%Y%m%d")
            print("[Crawler_Joins_Kwd] SecitonID:%s Data:%s" % (self.sectionID, self.curDate))
            urlPage = "https://news.naver.com/main/ranking/popularDay.nhn?rankingType=popular_day&sectionId=%s&date=%s" % (
            self.sectionID, self.curDate)
            self.openPage(urlPage)
    ...
    def scrape(self,rank,isShowContent):
        css_head = "tts_head"
        css_contents = "article_body"
        try:
            #1.date
...
def Main() :
    if False :
        ...
        MOD = crawler_naver_secRank_v2( False, "../Data/Text/Naver/Section/100" , "Naver_News_Sec_100" , "네이버_섹션_뉴스_종합" , 100 , '20180101', '20180828' )
        MOD.run()
