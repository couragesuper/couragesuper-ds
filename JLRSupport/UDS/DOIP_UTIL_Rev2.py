...
# worktodo
    # implementation about flow control.
        # last sending command
        # result or 0x78 (pending)
        # handle ack result
        #
...
# Configutaion File contains connection information.
xmlName = "Config.xml"

# Precondition : ba is valuable Array
def BaToVal( ba ) :
    value = 0
    if( type( ba ) == int ) : return True , ba  # if bytearray is short  then that is integer type.
    if( len( ba ) > 4 ) :
         return False, 0
    for elem in ba :
        value = (value << 8) | elem
...
class Doip_Util :
    def __init__(self):
        # flag for function of some debug
        self.debug = 1
    ...
    def makeDIDLists(self):
        self.arrDIDs = []
        self.arrDIDs.append(b'\xf1\x11')
        self.arrDIDs.append(b'\xf1\x12')
        self.arrDIDs.append(b'\xf1\x13')
        self.arrDIDs.append(b'\xf1\x88')
        self.arrDIDs.append(b'\xf1\x90')
        self.arrDIDs.append(b'\xf1\xA0')
        self.arrDIDs.append(b'\xf1\xBE')
        self.arrDIDs.append(b'\x41\xAE')
        self.arrDIDs.append(b'\x48\x84')
    ...
    def loopUDS_Send(self):
        ...
        while True :
            ...
            if False :
                ...
                if self.cnt < len(self.arrDIDs) :
                    self.udsReadingDID( self.arrDIDs[self.cnt] )
                    self.cnt = self.cnt + 1
                elif self.cnt == len(self.arrDIDs) :
                    self.cnt = 0
