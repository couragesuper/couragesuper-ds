...
config_db = {'user': 'root'
                     '', 'password': 'karisma*3%7*4', 'host': 'mthx.cafe24.com', 'database': 'bible', 'raise_on_warnings': True}
db  = dbConMysql(config_db)
db2 = dbConMysql(config_db)
dbUpdate = dbConMysql(config_db)
...
def updateTitle( iBook , iChap , szWord , szTitle ) :
    ...
    print( insertQry )
    if( dbUpdate.commitQuery( insertQry ) ) :
        return True
...
for iBook in range( 1 , len(data) + 1 ):
    ...
    for iChap in range( 1, cntChap + 1) :
        ...
        cntNode = 1
        for node in nodes :
            #print( "cntNode:{} node.tag {} , node.text {}".format( cntNode , node.tag_name,  node.text ) )
            if isTitle :
                hypers = node.find_elements_by_tag_name("a")
                if( len( hypers ) != 0 )  :
                    for hyper in hypers :
                        #print( "Title [{}] , Word:{} ".format( Title,  hyper.get_attribute("name") ) )
                        if( updateTitle( iBook , iChap , hyper.get_attribute("name") , Title ) ) :
                            isTitle = False
                            Title = ""
                            break
            if ( node.tag_name == "p" ) :
                isTitle = True
                Title = node.text
                #print("Title is configured with {}".format( cntNode ))
            cntNode = cntNode + 1
