...
def parseSearchEx( msg ) :
    print("msg:{}".format(msg))
    pats = [{"pat": "([가-힣]*)([0-9]*):([0-9]*)-([0-9]*):([0-9]*)", "type": 1},
    ...
    for pat in pats :
        ...
        if( m != None ) :
            ...
            if( pat['type'] == 1 ):
                return { "type":1 , "bookseq":bookseq, "bible_sunnm" : m.groups()[0] , "chap_start" : m.groups()[1] ,  "verse_start" : m.groups()[2] ,
                         "chap_end" : m.groups()[3] ,  "verse_end" : m.groups()[4]  }
            elif (pat['type'] == 2):
                return { "type":2 , "bookseq":bookseq, "bible_sunnm": m.groups()[0], "chap_start": m.groups()[1], "verse_start": m.groups()[2],
                         "verse_end": m.groups()[3]}
            elif (pat['type'] == 3):
