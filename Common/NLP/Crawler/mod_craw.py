...
def crawling_table_getlist_tag_xpath(driver, xpath, tag, isElemList):
    ...
    if isElemList == True:
        ...
        return [{"text": elem.text, "href": elem.get_attribute("href"), "elem": elem, "src": elem.get_attribute("src")}
                for elem in list_webelem]
...
def InitWebDriver(isLinux=False, isHidden=False):
    if (isLinux):
        path_chrome_driver = path_chrome_driver_linux
    else:
        path_chrome_driver = path_chrome_driver_win
    if (isHidden):
